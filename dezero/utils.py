...
def _dot_func(f):
    ...
    for y in f.outputs:
        txt += dot_edge.format(id(f), id(y()))# yはweakref, y()あり
    return txt
...
def plot_dot_graph(output, verbose=True, to_file='graph.png'):
    ...
    # 1.dotデータをファイルに保存
    tmp_dir = os.path.join(os.path.expanduser('~'), '.dezero')# ~/.dezeroディレクトリがなかったら作成
...
##the following is needed to understand! 2020/11/9
##
def sum_to(x, shape):
    """Sum elements along axes to output an array of a given shape.
    Args:
        x (ndarray): Input array.
        shape: The shape of Input array.
    Returns:
